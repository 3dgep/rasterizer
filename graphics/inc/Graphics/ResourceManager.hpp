...
/// <param name="blendMode">The blend mode to apply to the sprites. Default is the default BlendMode.</param>
/// <returns>The loaded sprite sheet, or null if the sprite sheet couldn't be loaded.</returns>
std::shared_ptr<SpriteSheet> loadSpriteSheet( const std::filesystem::path& filePath, uint32_t rows = 1, uint32_t columns = 1, uint32_t padding = 0, uint32_t margin = 0, const BlendMode& blendMode = BlendMode{} );
